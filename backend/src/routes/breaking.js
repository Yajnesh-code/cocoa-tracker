...
router.post('/', auth, async (req, res) => {
  ...
  if (Array.isArray(buckets)) {
    bucketRows = buckets.map(bucket => {
      if (bucket.type === 'good') {
        return {
          good_weight: bucket.weight !== undefined && bucket.weight !== null && bucket.weight !== ''
      ...
      if (bucket.type === 'bad') {
        return {
          good_weight: null,
          bad_weight: bucket.weight !== undefined && bucket.weight !== null && bucket.weight !== ''
            ? Number(bucket.weight)
            : null,
        };
      }
  ...
  if (bucketRows.some(row => (row.good_weight != null && Number.isNaN(Number(row.good_weight))) || (row.bad_weight != null && Number.isNaN(Number(row.bad_weight))))) {
