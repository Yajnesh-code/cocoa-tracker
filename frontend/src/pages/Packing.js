...
export default function Packing() {
  ...
  useEffect(() => {
    Promise.all([api.get('/batches'), api.get('/drying')])
      .then(([batchRes, dryingRes]) => {
        setBatches(batchRes.data);
        setDryingRecords(dryingRes.data);
      })
      .catch(() => setError('Unable to load packing data'));
  }, []);
  ...
  const dryingCompletedToday = new Set(
    dryingRecords
      .filter((item) => item.end_date && item.end_date.slice(0, 10) === today)
      .map((item) => String(item.batch_id))
  );
  const packableBatches = batches.filter((batch) => !batch.packed && dryingCompletedToday.has(String(batch.id)));
  ...
  return (
    <div>
      <div className="page-header">
        <h1>Packing Stage</h1>
        <p>Record final packing weights for batches that completed drying today.</p>
      </div>
            ...
            <div className="form-group">
              ...
              {bags.map((b, i) => (
                <div key={i} style={{ display: 'flex', gap: 8, marginBottom: 6 }}>
                  ...
                  />
                  {bags.length > 1 && (
                    <button type="button" className="btn btn-sm btn-danger" onClick={() => removeBag(i)}>X</button>
                  )}
                </div>
              ))}
